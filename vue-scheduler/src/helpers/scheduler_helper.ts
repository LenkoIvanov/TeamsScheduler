...
import { differenceInMinutes, isSameMinute, isWithinInterval, setSeconds } from 'date-fns';
...
export const calculateEightAmOffset = (startTime: Date): number => {
  return (startTime.getHours() * 60 + startTime.getMinutes() - eightAmOffset) * minuteMultiplier;
};

export const calculateSingularEventOffset = (event: EventInfo): EventOffset => {
  const eventHeight = differenceInMinutes(event.endTime, event.startTime) * minuteMultiplier;
...
export const getLastConcurrentEventIdx = (
  ...
  const isNextEventWithinCurrent =
    ...
    isEventWithinInterval(allEvents[currentEventIdx + 1], allEvents[currentEventIdx]);
